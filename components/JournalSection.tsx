        ...
        <div className="container-responsive max-w-7xl">
            ...
            <div className="grid md:grid-cols-3 gap-10">
                {posts.map((post, idx) => {
                    ...
                    return (
                        <div key={post.id} className="group cursor-pointer will-change-transform" onClick={() => onNavigate('blog')} style={{ transform: `translateY(${Math.max(-50, Math.min(50, cardOffset))}px)` }}>
                            <div className="aspect-[4/3] bg-stone-100 rounded-[1.5rem] overflow-hidden mb-6 border border-stone-200/50 relative shadow-lg group-hover:shadow-xl transition-all duration-500">
                                <img src={post.coverImage || 'https://images.unsplash.com/photo-1550684848-fac1c5b4e853?auto=format&fit=crop&w=600&q=80'} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 grayscale group-hover:grayscale-0 will-change-transform" alt={post.title} style={{ transform: `scale(${1 + Math.abs(cardOffset) * 0.0005})` }} />
                            </div>
