...
export const StoryShowcasePage: React.FC<StoryShowcasePageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-paper pt-32 pb-24 relative overflow-x-hidden">
      {/* 1. CINEMATIC HEADER */}
      <header className="max-w-7xl mx-auto px-8 md:px-16 mb-32">
      ...
      {/* 2. THE VIRTUAL BOOK (Side-by-Side) */}
      ...
      <section className="bg-stone-900 py-32 md:py-48 text-white relative overflow-hidden">
        {/* Subtle decorative grid in dark mode */}
        ...
        <div className="max-w-7xl mx-auto px-8 md:px-16 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-12">
            ...
            <p className="max-w-md text-stone-400 font-light text-lg leading-relaxed">
              Every story deserves a heartbeat. I transform static visual development into kinetic marketing assets that captivate readers before they even open the first page.
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-12 md:gap-24">
            {story.animations.map((anim, idx) => (
              <div key={idx} className="group cursor-pointer space-y-8" onClick={() => setActiveVideo(anim.videoUrl)}>
      ...
      {/* 4. CALL TO ACTION FOR AUTHORS */}
      <section className="py-32 md:py-48 max-w-7xl mx-auto px-8 md:px-16 text-center">
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <p className="text-stone-500 text-xl md:text-2xl font-light italic max-w-2xl mx-auto leading-relaxed">
              Let's collaborate to build an immersive universe for your characters. From cover design to cinematic trailers.
            </p>
            <div className="pt-8">
              <button 
                onClick={() => onNavigate('contact')}
                className="group inline-flex items-center gap-6 px-12 py-6 bg-stone-900 text-white rounded-full font-black uppercase tracking-[0.4em] text-xs hover:bg-accent hover:text-stone-900 transition-all shadow-2xl hover:-translate-y-2"
              >
      ...
      {/* VIDEO MODAL */}
      {activeVideo && (
        <div className="fixed inset-0 z-[1000] bg-stone-900/98 backdrop-blur-2xl flex items-center justify-center p-6 md:p-12 animate-in fade-in duration-500">
           <button 
             onClick={() => setActiveVideo(null)} 
             className="fixed top-8 right-8 z-[1010] p-4 text-stone-400 hover:text-white transition-all duration-500"
           >
             <X size={32} />
           </button>
           <div className="w-full max-w-6xl aspect-video bg-black rounded-3xl overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-white/5">
