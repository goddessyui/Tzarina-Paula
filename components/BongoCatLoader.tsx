...
export const BongoCatLoader: React.FC = () => {
  const containerRef = useRef<HTMLDivElement>(null);
  const { config } = useConfig();
  
  // Extract theme colors for easier usage
  const bg = config.theme.colorBackground;
  const text = config.theme.colorText;
  const accent = config.theme.colorAccent;
  ...
  useEffect(() => {
    ...
    // Selectors
    const s = (selector: string) => containerRef.current?.querySelectorAll(selector);
    const notes = s(".note");

    // Clone notes for more density
    if (notes) {
      notes.forEach((note) => {
    ...
    // Initial State
    gsap.set(musicNote, { scale: 0, autoAlpha: 1 });
    
    // Typing Animation (Simulating DrawSVG with opacity/stagger)
    gsap.fromTo(s(".terminal-code line"), 
      { opacity: 0, x: -5 },
      {
        opacity: 1,
        x: 0,
    ...
    // Paw Animation Helper
    const animatePawState = (selector: any) =>
      gsap.fromTo(
        selector,
        { autoAlpha: 0 },
        {
    ...
    // Note Animation Helper
    // Use a palette derived from the theme
    const palette = [accent, text, '#a8a29e', '#d6d3cd']; 
    const colorizer = gsap.utils.random(palette, true);
    ...
    // Split notes into groups and animate
    const noteEls = gsap.utils.toArray(musicNote);
    gsap.utils.shuffle(noteEls);
    
    const numNotes = Math.floor(noteEls.length / 3);
    const notesG1 = noteEls.slice(0, numNotes);
    const notesG2 = noteEls.slice(numNotes, numNotes * 2);
    const notesG3 = noteEls.slice(numNotes * 2);

    tl.add(animateNotes(notesG1), "start")
      .add(animateNotes(notesG2), "start+=0.5")
      .add(animateNotes(notesG3), "start+=1");
  ...
  return (
    <div 
      className="fixed inset-0 z-[200] flex flex-col items-center justify-center font-mono transition-colors duration-500"
      style={{ backgroundColor: bg, color: text }}
    >
      <div ref={containerRef} className="container-bongo relative w-[80vw] max-w-[600px] aspect-[783/355]">
        <style>{`
          .container-bongo svg {
            width: 100%;
            height: 100%;
          ...
          <g id="bongo-cat">
            ...
            <g className="music music-right">
              <g className="note">
                ...
                <g>
                  <path d="M368.5,46.5c.5,2.1,1.2,3.5,3.8,6.3s5.1,4.3,6.5,7.2a11.1,11.1,0,0,1,.7,2,10.5,10.5,0,0,1-.7,6.5" />
                  <path d="M368.5,46.5a20.8,20.8,0,0,0,2.4,11.7c2.4,4.3,5.1,5.4,6.8,9.5a17.5,17.5,0,0,1,.4,11" />
                  <line x1="368.5" y1="47.7" x2="368.5" y2="92.8" />
                  <path d="M368.5,92.8c.1-3.1-4.7-6.3-9-6.3s-8.7,2.7-8.7,5.8,4.8,5.7,8.7,5.8S368.3,95.8,368.5,92.8Z" />
                </g>
              </g>
            ...
            <g className="table">
              ...
              <line x1="25.3" y1="158.5" x2="783.2" y2="293" fill="none" strokeMiterlimit="10" strokeWidth="4" />
      ...
      <div 
        className="mt-8 text-sm uppercase tracking-[0.3em] font-bold animate-pulse font-body"
        style={{ color: accent }}
      >
        Initializing System...
      </div>
